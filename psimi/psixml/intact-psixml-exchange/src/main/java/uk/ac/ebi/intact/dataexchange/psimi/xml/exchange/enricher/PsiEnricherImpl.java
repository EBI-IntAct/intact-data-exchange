 ...
 * @version $Id$
 */
public class PsiEnricherImpl implements PsiEnricher {

    private EnricherContext enricherContext;
    private IntactEntryEnricher intactEntryEnricher;
    private InstitutionEnricher institutionEnricher;
    private InteractionEnricher interactionEnricher;

    public PsiEnricherImpl(EnricherContext enricherContext, IntactEntryEnricher intactEntryEnricher, InstitutionEnricher institutionEnricher, InteractionEnricher interactionEnricher) {
        this.enricherContext = enricherContext;
        this.intactEntryEnricher = intactEntryEnricher;
     ...
     * @throws IOException
     */
    public void enrichPsiXml(InputStream sourcePsi, Writer enrichedPsiWriter, EnricherConfig config) throws PsiEnricherException {
        ...
        try {
            ...
            for (Iterator<IndexedEntry> indexedEntryIterator = indexedEntries.iterator(); indexedEntryIterator.hasNext();) {
                ...
                institutionEnricher.enrich(institution);
                ...
                while (iterator.hasNext()) {
                    ...
                    interactionEnricher.enrich(intactInteraction);
    ...
    public Entry enrichEntry(Entry entry, EnricherConfig config) {
        EntryConverter converter = new EntryConverter();
        IntactEntry intactEntry = converter.psiToIntact(entry);

        enricherContext.setConfig(config);

        intactEntryEnricher.enrich(intactEntry);
    ...
    private void writeEntrySet(EntrySet entrySet, Writer destWriter) throws IOException {
