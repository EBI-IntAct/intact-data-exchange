...
#SBATCH -o "/nfs/production/hhe/intact/data/db-import-logs/import-complex-%j.out"   # job output file
...
then
      ...
      mvn -U clean install -Pimport,${PROFILE} -Dmi.file=${INPUT_FILE} -Djami.user.context.id=${USER_ID} -Djob.name=complexImport  -Dmaven.repo.local=repository -Derror.file=import_errors.log -Dmaven.test.skip
else
      echo ""
