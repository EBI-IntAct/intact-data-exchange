...
then
      ...
      mvn -U install -restart-import,${PROFILE} -Dmi.file=${INPUT_FILE} -Djami.user.context.id=${USER_ID} -Djob.name=interactionMixImport -Djob.id=${JOBID} -Derror.file=import_errors.log -Dmaven.test.skip -Dmaven.repo.local=repository
else
      echo ""
