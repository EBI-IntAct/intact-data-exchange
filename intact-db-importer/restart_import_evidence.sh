...
then
      ...
      mvn -U install -restart-import,${PROFILE} -Dmi.file=${INPUT_FILE} -Djami.user.context.id=${USER_ID} -Djob.name=interactionEvidenceImport -Djob.id=${JOBID} -Dmaven.test.skip
else
      echo ""
