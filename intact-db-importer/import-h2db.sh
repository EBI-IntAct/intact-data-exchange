...
then
      ...
      mvn -U clean install -import-h2,${PROFILE} -Dmi.file=${INPUT_FILE} -Djob.name=interactionMixImport -Dmaven.repo.local=repository -Derror.file=import_errors.log -Dmaven.test.skip
else
      echo ""
