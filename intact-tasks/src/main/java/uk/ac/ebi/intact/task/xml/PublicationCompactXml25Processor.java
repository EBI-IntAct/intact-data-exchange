...
public class PublicationCompactXml25Processor implements ItemProcessor<Publication, SortedSet<PublicationEntry>>, ItemStream {
    ...
    public void open(ExecutionContext executionContext) throws ItemStreamException {
        ...
        if (executionContext.containsKey(SEQUENCE_ID)){
            int currentId = executionContext.getInt(SEQUENCE_ID);
            IdSequenceGenerator.getInstance().resetIdTo(currentId);
        }
        else {
    ...
    public void update(ExecutionContext executionContext) throws ItemStreamException {
        int currentId = IdSequenceGenerator.getInstance().currentId();

        // we can persist the current position
        executionContext.putInt(SEQUENCE_ID, currentId);
    }
